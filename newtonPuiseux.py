...
currentPoly=None
solutions={}
# {first term:[#terms n, p_n-1, highest exponent,1st term + 2nd term + 3rd term + ... + nth term]}
def initialTerms(poly):
    ...
    for key in slopePoly:
        ...
        for coeff in slopePoly[key]:
            ...
            solutions[firstTerm]=[1,poly,-key,firstTerm]
    return solutions.keys()
...
def expand(poly,initialTerm,n=4,showPlot=False):
    ...
    if numTerms == -1: # placeholder for when the series is fully computed
        return solutions[initialTerms][3]
    while numTerms < n:
        # plugging (y+terms) into poly
        toPlug = solutions[initialTerm][3].TT()
...
if __name__=='__main__':
    ...
    if n==0:
        ...
        print
        print expand(p,first,5,showPlot=False)
    elif n==1:
        ...
        #terms = puiseux({2:-2,3:-16,4:-224,5:-3840,6:-39504})
        print terms
        print 'p(terms): ',p(terms)
        print p(first)
    elif n==2:
        ...
        p = mypoly({0:puiseux({0:1}),2:puiseux({1:1})})
        print initialTerms(p)
